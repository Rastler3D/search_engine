...
use analyzer::token::{OwnedToken, SeparatorKind};
...
impl MatcherBuilder {
    pub fn new(matching_words: MatchingWords, analyzer: BoxAnalyzer) -> Self {
        Self {
            matching_words,
            analyzer,
            crop_marker: None,
    ...
    pub fn build<'t, 'm>(&'m self, text: &'t str) -> Matcher<'t, 'm> {
...
pub struct Matcher<'t, 'm: 't> {
    text: &'t str,
    matching_words: &'m MatchingWords,
    analyzer: &'m BoxAnalyzer,
    crop_marker: &'m str,
    highlight_prefix: &'m str,
    highlight_suffix: &'m str,
    matches: Option<(Vec<OwnedToken<'t>>, Vec<Match>)>,
}

impl<'t, 'm: 't> Matcher<'t, 'm> {
    ...
    fn compute_matches(&mut self) -> &mut Self {
        ...
        fn compute_partial_match<'a, 'c:'a>(
            mut partial: PartialMatch,
            token_position: usize,
            word_position: usize,
            words_positions: &mut impl Iterator<Item = (usize, usize, &'a OwnedToken<'c>)>,
        ...
        let tokens = self.analyzer.analyze(self.text).as_iter().collect::<Vec<_>>();
    ...
    pub fn matches(&mut self) -> Vec<MatchBounds> {
        match &self.matches {
            ...
            Some((tokens, matches)) => matches
                .iter()
                .map(|m| MatchBounds {
                    start: tokens[m.token_position].offset_from,
                    length: m.match_len,
                })
                .collect(),
        }
    ...
    fn crop_bounds(&self, tokens: &[OwnedToken], matches: &[Match], crop_size: usize) -> (usize, usize) {
        ...
        let crop_byte_start = before_tokens.next().map_or(0, |t| t.offset_to);
        let crop_byte_end = after_tokens.next().map_or(self.text.len(), |t| t.offset_from);
                ...
                Some((tokens, matches)) => {
                    ...
                    if format_options.highlight {
                        ...
                        for m in matches {
                            ...
                            if token.offset_from < byte_start || token.offset_to > byte_end {
                                continue;
                            }

                            if byte_index < token.offset_from {
                                formatted.push(&self.text[byte_index..token.offset_from]);
                            }

                            let highlight_byte_index = self.text[token.offset_from..]
                                .char_indices()
                                .enumerate()
                                .find(|(i, _)| *i == m.match_len)
                                .map_or(token.offset_to, |(_, (i, _))| i + token.offset_from);
                            formatted.push(self.highlight_prefix);
                            formatted.push(&self.text[token.offset_from..highlight_byte_index]);
                            formatted.push(self.highlight_suffix);
                            // if it's a prefix highlight, we put the end of the word after the highlight marker.
                            if highlight_byte_index < token.offset_to {
                                formatted.push(&self.text[highlight_byte_index..token.offset_to]);
                            }

                            byte_index = token.offset_to;
                        }
                    }
